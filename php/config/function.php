...
function insertComplain($con, $ordered_id, $cart)
{
    $query = "INSERT INTO enquiries (name, email,  message) VALUES (?, ?, ?)";
    $stmt = $con->prepare($query);
...
function getCurrentUser($con) {
    ...
    $query = $con->prepare("SELECT id, FirstName, LastName, Email, Role, IsBan FROM users WHERE id = ?");
...
function getCurrentUserField($con, $field) {
    $user = getCurrentUser($con);
    return $user ? ($user[$field] ?? null) : null;
}

function getCurrentUserName($con) {
    $user = getCurrentUser($con);
    if ($user) {
